...
VertexArray loadOBJModelFromFile(const char* filename)
{
    ...
    if(tokens.size() == 4)
    {
      ...
      {     
        for(int i = 0; i < 4; i++)
        {
          ...
          if(face.size() == 3)
          {            
            GLushort indice = static_cast<GLushort>(std::strtoul(face[0].c_str(), NULL, 0)) - 1;      
  ...
  for(v = vertices.begin(), v1 = vertex_list.begin(); v < vertices.end(); v++, v1++)
  {
    v1->position = *v;
  }

  return VertexArray(GL_TRIANGLES, 
      reinterpret_cast<GLvoid*>(vertex_list.data()), vertex_list.size(), indices.data(), indices.size());
}
...
bool compileShader(Shader* shader, const std::string& source)
{
  ...
  {
    std::cout << "=======================================" << std::endl;
    std::cout << "Shader compile error: (" << shader->getID() << ")" << std::endl;
...
void display()
{
  ...
  glDrawElements(terrain.getPrimitive(),terrain.getIndexCount(), GL_UNSIGNED_SHORT,0);
